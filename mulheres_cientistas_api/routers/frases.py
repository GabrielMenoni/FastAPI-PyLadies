...
def create_Frase(frase_to_create: FraseCreate,
                 session: Session = Depends(get_session)):
  try:
    return create(session=session, model=Frase, data=frase_to_create)
  except IntegrityError:
    raise HTTPException(status_code=422,
                        details="Cientista especificada não existe")
...
def get_frase(frase_id: int, session: Session = Depends(get_session)):
...
def update_frase(
    frase_id: int,
    frase_to_update: FraseUpdate,
    session: Session = Depends(get_session),
):
  frase = update(session=session,
                 model=Frase,
                 id=frase_id,
                 data=frase_to_update)
  if frase is None:
    raise HTTPException(status_code=404, detail="Frase não encontrada")
  return frase
