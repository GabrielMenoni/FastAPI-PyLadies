...
def create_cientista(cientista_to_create: CientistaCreate,
                     session: Session = Depends(get_session)):
  cientista_id = create(session=session,
                        model=Cientista,
                        data=cientista_to_create).id

  for frase_to_create in cientista_to_create.frases:
    create(session=session,
           model=Frase,
           data=FraseCreate(frase=frase_to_create, cientista_id=cientista_id))
...
def get_cientista(cientista_id: int, session: Session = Depends(get_session)):
  cientista = get_one(session=session, model=Cientista, id=cientista_id)
  if cientista is None:
    raise HTTPException(status_code=404, detail="Cientista não encontrado")
  return cientista
...
def update_cientista(
    cientista_id: int,
    cientista_to_update: CientistaUpdate,
    session: Session = Depends(get_session),
):
  cientista = update(session=session,
                     model=Cientista,
                     id=cientista_id,
                     data=cientista_to_update)
  if cientista is None:
    raise HTTPException(status_code=404, detail="Cientista não encontrado")
  return cientista
...
def delete_cientista(cientista_id: int,
                     session: Session = Depends(get_session)):
